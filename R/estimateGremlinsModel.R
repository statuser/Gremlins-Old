...
#'
estimateGremlinsModel <- function(data, design, Priors = NULL, R, nSegments = 2, covariates = NULL, constraints = NULL, segmentCovariates = NULL, startingValues = NULL) {
  ...
  if(!is.null(Priors)) {
    ...
    print(gremlinsEnv$nParams)
    print(gremlinsEnv$nCovariates)
    Priors = list(
        ...
        V_not = mu_not*diag(gremlinsEnv$nParams),
  ...
  if(is.null(startingValues)) {
    startingValues = list(slope = matrix(0, nrow = gremlinsEnv$nUnits, ncol = nParams),
      slopeBar = double(nParams),
      slopeCov = diag(nParams),
  ...
  slopeBar <- matrix(0, nrow=R, ncol=gremlinsEnv$nParams)
  slopeBar[1,] <- startingValues$slopeBar

  slopeCov <- array(0, dim=c(R, gremlinsEnv$nParams, gremlinsEnv$nParams))
  slopeCov[1,,] <- startingValues$slopeCov

  lambda <- matrix(0, nrow = R, ncol = gremlinsEnv$nSegments)
  lambda[1,] <- startingValues$lambda

  segMembership <- matrix(0, nrow = R, ncol = gremlinsEnv$nUnits)
  segMembership[1,] <- startingValues$segMembership
  ...
  for(rep in 2:R) {
   ...
   for(ind in 1:gremlinsEnv$nUnits) {
     ...
     slope[rep, ind, ] <- generateIndividualSlope(respData, respDesign,
         slope[rep - 1, ind, ], lambda[rep-1, segMembership[rep-1,ind]],
         slopeBar[rep-1,], slopeCov[rep-1,,], constraints)
         segMembership[rep, ind] <- generateSegmentMembership(respData, respDesign, slope[rep, ind, ], lambda[rep - 1,], phi_lambda[rep - 1,], Priors)
   }
    multiregOut <- multivariateRegression(slope[rep,,], covariates, Priors)
    slopeBar[rep,] <- multiregOut$Beta
    slopeCov[rep,,] <- multiregOut$Sigma    
    lambda[rep,] <- generateLambda(lambda[rep - 1,], data, design, slope[rep,,], segMembership[rep-1,], slopeBar[rep,], slopeCov[rep,,], Priors)
    phi_lambda[rep,] <- drawPhi(K[rep,], Priors$psi_k)
  }

  #; Save the output

  return(list(slope = slope, slopeBar = slopeBar, slopeCov = slopeCov, lambda = lambda, segmentMembership = segMembership))
...
multivariateRegression <- function(data, design, priors) {
  ...
  BTilde = chol2inv(chol(crossprod(R))) %*% crossprod(R, T)
...
# generateLambda <- function(currentLambda, data, design, currentSlope, currentSegment, currentSlopeBar, currentSlopeCov, priors) {
...
#       while(proposedLambda < lambda[k-1] || proposedLambda < 10 || proposedLambda > 100) {
#         proposedLambda <- lambda[k] + ((lambda[k] - lambda[1])/(6*gremlinsEnv$nSegments)) * rnorm(1, 0, 1)
#       }
#     }
#
...
estimateGremlinsModel <- function(data, design, Priors = NULL, R, nSegments = 2, covariates = NULL, constraints = NULL, segmentCovariates = NULL, startingValues = NULL) {
  ...
  slope <- array(0, dim=c(R, gremlinsEnv$nUnits, nParams))
  slope[1,,] <- startingValues$slope

  slopeBar <- matrix(0, nrow=R, ncol=nParams)
  slopeBar[1,] <- startingValues$slopeBar

  slopeCov <- array(0, dim=c(R, nParams, nParams))
  slopeCov[1,,] <- startingValues$slopeCov
  ...
  for(rep in 2:R) {
   ...
   for(ind in 1:gremlinsEnv$nUnits) {
    ...
    slopeBar[rep,] <- multiregOut$Beta
    slopeCov[rep,,] <- multiregOut$Sigma
    # slopeBar[rep, ] <- startingValues$slopeBar
    # slopeCov[rep,,] <- startingValues$slopeCov
    lambda[rep,] <- generateLambda(lambda[rep - 1,], data, design, slope[rep,,], segMembership[rep-1,], slopeBar[rep,], slopeCov[rep,,], Priors)
    segMembership[rep,] <- generateSegmentMemberships(data, design, slope[rep,,], slopeBar[rep,], slopeCov[rep,,], lambda[rep,])
  }
...
multivariateRegression <- function(data, design, priors) {
  U = chol(priors$Ainv)
  R = rbind(design, U)
  T = rbind(data, U %*% priors$mu_not)
  BTilde = chol2inv(chol(crossprod(R))) %*% crossprod(R, T)

  S = crossprod(T - R%*%BTilde)
...
generateLambda <- function(currentLambda, data, design, currentSlope, currentSegment, currentSlopeBar, currentSlopeCov, priors) {
  ...
  for(k in 2:gremlinsEnv$nSegments) {
    ...
    cIndLL <- double(gremlinsEnv$nUnits)
    pIndLL <- double(gremlinsEnv$nUnits)
    cIndPrior <- double(gremlinsEnv$nUnits)
    pIndPrior <- double(gremlinsEnv$nUnits)
    ...
    for(ind in 1:gremlinsEnv$nUnits) {
      if(currentSegment[ind] == k) {
        ...
        cIndPrior[ind] <- (priors$lambdaShape - 1) * log(lambda[k]) - lambda[k]/priors$lambdaScale
        pIndPrior[ind] <- (priors$lambdaShape - 1) * log(proposedLambda) - proposedLambda/priors$lambdaScale
      }
    }
...
generateSegmentMemberships <- function(data, design, slope, slopeBar, slopeCov, lambda) {
  segMem <- double(gremlinsEnv$nUnits)
  for(ind in 1:gremlinsEnv$nUnits) {
    prob <- double(gremlinsEnv$nSegments)
    slopeDiff <- slope[ind,] - slopeBar
    ssSlope <- t(slopeDiff)%*%solve(slopeCov, slopeDiff)
    for(k in 1:gremlinsEnv$nSegments) {
      prob[k] = gremlinsLogLikelihood(data[ind, -c(1:2)], design[design[,1] == data[ind, 2], -c(1:3)], slope[ind,], lambda[k])
      prob[k] = prob[k] - 0.5*ssSlope
    }

    prob <- exp(prob)
    prob <- prob/sum(prob)
    segMem[ind] <- sample(1:gremlinsEnv$nSegments, 1, prob = prob)
  }
